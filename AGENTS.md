...
* Planned row-wise/schema operations (`head`, `tail`, `filter`, `transform`, `group`, `reduce`, `select`, `sort`, `rename`, `remove`, `distinct`, `count`, `describe`, and `join`) are planned as one schema-planned pipeline. For prepared primary sources, the phase split is `prepared source schema -> typed logical plan including source -> optimized logical IR -> physical plan including source bindings -> source stream or materialized source load -> streaming/materialized execution`. Typed logical planning validates operation-specific schema rules, type-checks expressions, prepares join right-hand source schemas when needed, and computes each next schema without inspecting left-side rows beyond the prepared source schema. Join right rows are loaded later from the physical right-source spec. Schema acquisition and planning errors happen before execution/materialization errors; for example, a missing join file can be reported before a primary source row outside the inference window fails conversion.
* Planner schema environments are immutable values built through central constructors, not ad-hoc parallel slice pairs. A valid environment is an ordered list of `name + raw planning schema` column descriptors plus an optional derived `map[string]int` name index. Because each descriptor carries its own name and schema, a name/schema length mismatch is not representable inside `schemaEnv`. Column names are unique, and any stored index is extensionally equal to the ordered descriptors (`index[name] = i` iff `columns[i].name == name`). Column lookup must return an explicit `schemaEnvColumnRef, ok` proof rather than an in-band sentinel such as `-1`, `""`, or `nil`; callers that already range over the ordered descriptors should use that descriptor directly. Finalized planning schemas are derived from raw schemas on demand. Tiny environments validate names with the ordered descriptors directly; checked non-tiny environments retain a plain map cache for top-level lookup, while internally proven derived environments may rely on ordered lookup to avoid unnecessary planning allocations. Planner-derived environments built from already proven-unique columns are intentionally unindexed unless a caller has enough immediate lookups to justify the cache; source/external schema boundaries use checked constructors. Constructor failures at external schema boundaries must be returned as planning errors; optimization eligibility probes may convert a constructor failure to an ineligible `false` result only when the normal validated planning path remains available. Must-wrappers and unchecked constructors are reserved for internally proven-unique derived environments, including user-named outputs after planner duplicate-name checks have already established uniqueness. Public output order must always come from the ordered descriptor list, never map iteration order.
* `schemaEnv` is the primary internal proof of planner output shape. Every logical and physical planned operation exposes `OutputEnv() schemaEnv`; `OutputSchema()` is a compatibility/boundary rendering of that proof for table construction, stream schema APIs, public test helpers, and user-visible output metadata. Planner bases must not store both a `schemaEnv` and a `table.Schema` authority, because that would make disagreement representable. Schema-preserving operations should pass through the existing environment proof when their output shape is unchanged. Schema-changing operations must construct exactly one new valid environment. A `plannedRowSpan` takes its output environment structurally from its final child, so the span cannot report a different output shape from the last row-local operation it contains.
* Logical transform and reduce planning must use find-or-append helpers that return an explicit column reference for output writes. Physical planning resolves assignment targets from the final output environment by name; that lookup must return `(ref, ok)` and handle absence explicitly. It must not encode missing targets as `-1` and then rely on callers to remember which integers are safe indexes.
* The optimizer has four ordered rewrites. First, an adjacent prefix made only of eligible `select` and `filter` operations may be represented as source output columns plus source predicates for prepared primary single-file sources. This prefix rewrite is format-agnostic: it reads only the source schema and logical operators, never CSV/JSON/Parquet details. It is valid for top-level identity projections with unchanged output names (`select id, status`, not `select id, id` or `select address.city`) and source-safe filters. A source-safe filter is composed only from top-level columns, literals, comparisons (`==`, `!=`, `<`, `>`, `<=`, `>=`), boolean `and`/`or`/`not`, and `is null` / `is not null`; function calls, arithmetic, nested paths, constructors, and other expression forms stay as normal pipeline filters. Eligible select/filter operations are removed from the optimized operation list only when the source fully satisfies them. The prefix rewrite stops scanning at the first unsupported operation and does not push later `select` or `filter` operations across it. Second, adjacent `logicalGroup` followed immediately by `logicalReduce` over the group's nested column is replaced with `logicalGroupReduce`. Standalone `group`, non-adjacent group/reduce, and explicit-list `reduce orders ...` remain on the compatibility path. Third, demand-driven pruning walks the optimized logical pipeline backward. Fourth, row-local logical runs of two or more adjacent operations are grouped into explicit `optimizedRowSpan` nodes.
* Demand-driven pruning runs before row-span fusion and computes the set of top-level input columns demanded by the final output and each operation. It is a semantic rewrite over logical names and schemas only. `filter` demands its predicate columns plus demanded pass-through columns; `sort` demands sort keys; `select` maps demanded output names back to source paths only when those selected outputs are themselves demanded downstream; `rename` maps demanded output names back positionally; `remove` intersects demand with kept columns; `transform` keeps only demanded assignments and demands their right-hand-side inputs while preserving simultaneous assignment semantics; `count` demands no data columns. Therefore `select bad_col | count` and `transform unused = erroring_expr | select name | count` may not evaluate the projected value or dead assignment. Join demand is a product transfer: left input demand is left key roots plus demanded left-side output roots, and right source demand is right key roots plus demanded right-side output roots. Join keys are semantic demand for all join kinds, including `count` after a join, because join cardinality and outer-row emission depend on them. Demanded right-side output columns do not demand same-named left columns. The logical join carries semantic output-source metadata from the original full join schema so right-side collision names remain exactly the names produced by the unoptimized logical pipeline. Logical and optimized joins carry a concrete `PreparedJoinSource` capability, its schema environment, key bindings, output-source witnesses, and right column demand; they must not carry loaded right rows, load options, or a separate provider/source-ID pair. Load options are consumed by `PrepareJoinSource` when it builds the prepared loader; the resulting capability carries only source identity for diagnostics, schema proof, and the projection-only load operation. Public `PreparedJoinSource` construction defensively snapshots schema before delegating to the private snapshot constructor, while engine-owned providers transfer already-snapshotted metadata to that constructor so planning does not clone the same proof twice. A nil prepared loader is rejected at the single build point before a capability exists; `Load` should not carry a second nil-loader branch. Physical planning turns that metadata into left/right output bindings plus a projection-only right-hand `JoinSourceLoadSpec`, and execution materializes the finite right side by calling the prepared source capability with that spec. Fused group/reduce always demands group key paths. If the original nested payload output is demanded, it demands all fused input columns and materializes the payload. If the payload is not demanded, it demands only live aggregate argument roots plus group key roots, and dead reduce assignments may be removed. A reduce assignment that overwrites the nested column name, such as `reduce grouped = count()`, is an aggregate output demand rather than a payload demand. `describe`, full-row `distinct`, standalone `group`, and explicit-list `reduce` are read-all barriers. Projected `distinct` keeps all projection keys because dropping an apparently unused key would change distinct row identity.
* Row-span fusion is a late optimized-IR normalization. `optimizedRowSpan` contains a maximal run of at least two already schema-planned row-local logical operations: `logicalFilter`, `logicalSelect`, `logicalRename`, `logicalRemove`, and `logicalTransform`. Singleton row-local operations remain as their existing plan variants so cheap one-op pipelines do not allocate wrapper spans. A span preserves child order and each child's schema boundary. It must not include `head`, `tail`, `sort`, `distinct`, `group`, `reduce`, `logicalGroupReduce`, `join`, `count`, or `describe`, and it must not store physical indexes, compiled evaluators, or source read-set facts. Because it runs after demand pruning, dead transform assignments, identity selects, no-op renames/removes, source read-set decisions, and fused group/reduce payload decisions are already made before a span is formed.
...
* Format-specific source stream code consumes the physical source spec. Output columns and predicate columns are validated/emitted before the pushed predicate is evaluated, so bad records in those columns remain observable even when the predicate would reject the row. Type/schema errors in truly unreferenced columns are skipped on the optimized single-file source path. For JSON/JSONL, "skipped" here means skipped from schema validation and table materialization after the logical JSON record has been parsed; the current decoder still constructs the demanded row's field values. Primary glob and CLI stdin sources disable source column pruning and source predicate pushdown, read all source columns, and keep existing bad-record visibility. Right-hand join sources, including deterministic globs, consume their join-specific physical source spec. Source structure validation such as duplicate headers and row-width checks remains observable independent of the read set.
...
* Typed logical and optimized logical IRs carry semantic facts only: column paths/identities, expression result schemas, function signatures, operation ordering, operation output environments, join output source identities, source output-column requirements, and source logical predicates. They must not contain stage-local table column indexes, projection index lists, transform/reduce target indexes, join key indexes, physical join output maps, physical source read sets, loaded join rows, or compiled evaluators. Logical join planning schema-prepares the right-hand source for schema acquisition and key validation; right-hand rows are materialized only during execution from the physical join source spec.
* Physical planning happens after the optimizer. It assigns executor-ready metadata such as top-level column indexes, projection index lists, bound sort/filter/group/join accessors, transform/reduce target indexes, fixed `count`/`describe` output environments, rename/remove result shapes, join key bindings, join output column maps, join output environments, fused group/reduce aggregate state slots, and compiled evaluator choices. `optimizedRowSpan` lowers to a single `plannedRowSpan` by physicalizing each child against the previous child's output environment, so renames/removes/selects/transforms inside the span cannot see columns that the preceding child has hidden.
...
* Streaming execution consumes explicit `plannedRowSpan` nodes, singleton row-local physical nodes, and materialization boundaries. It must not rediscover row-local runs by scanning arbitrary adjacent planned operations. `filter`, `select`, `remove`, `rename`, and row-local `transform` children inside a planned span compile into one row program; singleton row-local nodes compile directly into their one row step. Eligible spans or singleton row-local nodes may run with bounded ordered parallelism, but output rows and runtime errors must remain in input order. `head` is a streaming boundary rather than a parallel row program because it must not read ahead.
* `count` and `describe` are bounded-memory folds: they consume the upstream stream before emitting their small result table. `tail`, `sort`, `distinct`, `group`, `reduce`, fused `group_reduce`, and `join` are materialization boundaries and then run through the existing materialized executor. Fused `group_reduce` scans input rows once, keeps first-seen group order, updates aggregate state slots directly, and builds nested payload records only when demanded. After a materialized boundary finishes, the result table can re-enter row streaming for later row-local spans or singleton row-local operations. Non-dropping row-local spans and singleton row-local operations immediately before a materialized boundary may run through the materialized table interpreter to keep table fast paths such as append-only typed transform.
* Prepared single-file source execution is adaptive. If the physical plan has an early streaming benefit (`head`, bounded folds, or a row-dropping row-local operation/span before materialization), it opens the native source row stream. If the plan would immediately materialize before a blocker without dropping rows, it uses the prepared materialized loader path and then enters the same streaming/materializing executor over the resulting table. This preserves source projection/predicate semantics while avoiding generic stream materialization on table-fast-path workloads. Adaptive source choice must inspect `plannedRowSpan` and singleton row-local metadata rather than reconstructing spans from adjacent physical ops.
