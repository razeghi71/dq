...
* Planned row-wise/schema operations (`head`, `tail`, `filter`, `transform`, `group`, `reduce`, `select`, `sort`, `rename`, `remove`, `distinct`, `count`, `describe`, and `join`) are planned as one schema-planned pipeline. A pipeline plan is `input schema + raw ops -> planned ops + output schema` before row execution. Planning binds paths, validates operation-specific schema rules, type-checks expressions, loads join right-hand sources when needed, and computes each next schema without inspecting left-side rows.
* Pipeline plan execution must consume planned metadata instead of rediscovering schemas or rebinding paths. This includes projection output names and schemas, bound sort/filter/group/join metadata, typed transform expressions and target indexes, typed reduce aggregate expressions and target indexes, fixed `count`/`describe` schemas, rename/remove result shapes, loaded join right tables, join key bindings, join output column maps, and join output schemas.
