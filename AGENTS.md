...
* Logical transform and reduce planning must use find-or-append helpers that return an explicit column reference for output writes. Physical planning may resolve assignment targets from the final output schema by name, but that lookup must return `(index, ok)` and handle absence explicitly; it must not encode missing targets as `-1` and then rely on callers to remember which integers are safe indexes.
