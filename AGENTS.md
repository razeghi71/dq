...
* The optimizer must not reorder operations, push predicates across transforms/joins/groups/sorts, eliminate group/reduce payload columns, or move runtime errors from demanded expressions across semantic barriers. Dead transform assignments whose outputs are not demanded may be removed, so their runtime errors intentionally disappear. Optimized rewrites must allocate replacement optimized nodes instead of mutating typed logical plan internals; no-op portions may share immutable logical facts to avoid planning-time allocation regressions.
