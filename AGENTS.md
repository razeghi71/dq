...
* Planned row-wise/schema operations (`head`, `tail`, `filter`, `transform`, `group`, `reduce`, `select`, `sort`, `rename`, `remove`, `distinct`, `count`, `describe`, and `join`) are planned as one schema-planned pipeline. The current phase split is `input schema + raw ops -> typed logical plan -> optimized logical IR -> physical plan -> execution`. Typed logical planning validates operation-specific schema rules, type-checks expressions, loads join right-hand sources when needed, and computes each next schema without inspecting left-side rows.
* The optimizer is currently a semantic no-op. It must return a distinct optimized logical IR wrapper, but it must not reorder operations, remove unused computations, push predicates/projections, or prune source columns. Because it is no-op, current runtime error ordering and loader bad-record behavior remain observable. The no-op optimized wrapper may share typed logical facts with the logical plan to avoid planning-time allocation regressions; those facts are immutable after logical planning. Future optimizer rewrites must allocate replacement optimized nodes instead of mutating shared logical plan internals.
* Typed logical and optimized logical IRs carry semantic facts only: column paths/identities, expression result schemas, function signatures, operation ordering, and operation output schemas. They must not contain stage-local table column indexes, projection index lists, transform/reduce target indexes, join key indexes, join output maps, or compiled evaluators. With the current loader API, logical join planning materializes the right-hand source once so it can validate schemas and preserve load-error ordering; that loaded source is not a physical binding.
...
```
tokens -> raw AST -> typed logical expression/plan -> optimized logical IR -> physical expression/plan -> row execution
```
